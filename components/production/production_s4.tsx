...
const ProductionS4 = ({ dict }: ProductionS4Props) => {
    return (
        <div className={"bg-[#0D3E29] py-32"}>
            <div className="container">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                    <div>
                        <h3 className={"text-[#fff] text-xl md:text-5xl uppercase mb-10 font-semibold"}>{dict.title}</h3>
                        {dict.paragraphs.map((p:string, i:number) => (
                            <React.Fragment key={i}><p className={"text-white"}>{p}</p>{i < dict.paragraphs.length - 1 && <br />}</React.Fragment>
                        ))}
                    </div>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-5 text-white mt-10 md:mt-0">
                        {dict.certificates.map((cert:ProductionDictionary['s4']['certificates'][number]) => (
                            <div key={cert.title} className="w-full min-h-[10rem] bg-[#000] rounded-2xl p-5 flex flex-col justify-between">
                                <div>
                                    <h5 className="text-2xl font-semibold mb-4">{cert.title}</h5>
                                    <p>{cert.subtitle}</p>
                                </div>

                                <a href={`/${cert.pdf_name}.PDF`} target={"_blank"}  className="flex items-center justify-between gap-2.5 rounded-full border border-white/[0.5] bg-transparent max-w-[150px] py-1 px-3 mt-5 group hover:bg-[#0D3E29] text-white transition-colors duration-300 text-sm font-medium cursor-pointer">
                                    {dict.downloadButton}
